...
import pytest
import my_sacn
from my_sacn.sender import check_universe
from my_sacn.messages.data_packet import DataPacket
from my_sacn.sending.sender_socket_test import SenderSocketTest
...
def test_constructor():
    ...
    sender = my_sacn.sACNsender(socket=socket)
    ...
    sync_universe = 4567
    sender = my_sacn.sACNsender(bind_address, bind_port, source_name, cid, fps, universe_discovery, sync_universe, socket)
...
def test_flush():
    socket = SenderSocketTest()
    sync_universe = 1234
    sender = my_sacn.sACNsender(sync_universe=sync_universe, socket=socket)
